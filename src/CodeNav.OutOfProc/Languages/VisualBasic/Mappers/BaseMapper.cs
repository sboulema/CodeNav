...
public static class BaseMapper
{
    ...
    private static CodeItemAccessEnum MapAccess(SyntaxTokenList? modifiers, SyntaxNode source)
    {
        ...
        if (modifiers.Value.Any(m => m.RawKind == (int)SyntaxKind.NotOverridableKeyword))
        {
        ...
        if (modifiers.Value.Any(m => m.RawKind == (int)SyntaxKind.FriendKeyword))
        {
    ...
    private static CodeItemAccessEnum MapDefaultAccess(SyntaxNode source)
    {
        if (source.Parent.IsKind(SyntaxKind.CompilationUnit))
        {
            ...
            {
                SyntaxKind.EnumBlock => CodeItemAccessEnum.Public,
                SyntaxKind.NamespaceBlock => CodeItemAccessEnum.Public,
        ...
        return source.Kind() switch
        {
            SyntaxKind.NamespaceBlock => CodeItemAccessEnum.Public,
            SyntaxKind.EnumBlock => CodeItemAccessEnum.Public,
            SyntaxKind.InterfaceBlock => CodeItemAccessEnum.Public,
