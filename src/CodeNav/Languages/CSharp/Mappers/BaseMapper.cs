...
public class BaseMapper(TextViewService textViewService)
{
    public T MapBase<T>(SyntaxNode source, SyntaxToken identifier, SyntaxTokenList modifiers,
    ...
    public T MapBase<T>(SyntaxNode source, NameSyntax name,
        SemanticModel semanticModel, CodeDocumentViewModel codeDocumentViewModel) where T : CodeItem
        => MapBase<T>(source, name.ToString(), [], semanticModel, codeDocumentViewModel);

    public T MapBase<T>(SyntaxNode source, string name,
        SemanticModel semanticModel, CodeDocumentViewModel codeDocumentViewModel) where T : CodeItem
        => MapBase<T>(source, name, [], semanticModel, codeDocumentViewModel);

    public T MapBase<T>(SyntaxNode source, SyntaxToken identifier,
        SemanticModel semanticModel, CodeDocumentViewModel codeDocumentViewModel) where T : CodeItem
        => MapBase<T>(source, identifier.Text, [], semanticModel, codeDocumentViewModel);

    private T MapBase<T>(SyntaxNode source, string name, SyntaxTokenList modifiers,
        SemanticModel semanticModel, CodeDocumentViewModel codeDocumentViewModel) where T : CodeItem
    {
        ...
        codeItem.TextViewService = textViewService;
